...
- **2026-08-11** — Added `RelatedAlertsBlock.rule_fired_count: int | None` (`history.py`) per enrichment-05-spec.md §13.2. Architecture §5.6 deducts 25 points from the history component when `rule_fp_rate >= 0.8` **and** `fired_count >= 10`; the envelope exposed only the rate, so the deduction's precondition was invisible to the analyst reading the output. Additive and defaulted — no existing shape changed, `SCHEMA_VERSION` unchanged. `make schema` regenerated.
- **2026-08-11** — Added `RelatedAlertsBlock.shared_entity_count: int` (`history.py`) per enrichment-05-spec.md §12.5. Architecture §5.6 adds 10 points when at least 3 related alerts *share an entity*, but `count` also includes rule-only matches (40 vs 5 on fixture 08, 12 vs 1 on fixture 04) and would over-award it. `prior_true_positives` / `prior_false_positives` were narrowed to entity-sharing matches in the same change — semantics only, no field shape change. Additive and defaulted — `SCHEMA_VERSION` unchanged. `make schema` regenerated.
